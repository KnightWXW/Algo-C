...
//      有 n 座山排成一列，每座山都有一个高度。
//      给你一个整数数组 height，其中 height[i] 表示第 i 座山的高度，
//      再给你一个整数 threshold 。
...
int main()
{
    ...
    int threshold = GenerateRandomNum(1, 100);
    PrintVecElement(vec, n);
    int rs = 0;
    int* ans_A = StableMountains(vec, n, threshold, &rs);
    printf("threshold 为 %d 时, 找到稳定山的下标 为: \n", threshold);
...
int *StableMountains(int *height, int heightSize, int threshold, int *returnSize)
{
    ...
    memset(ans, 0, sizeof(int) * heightSize);
    int index = 0;
