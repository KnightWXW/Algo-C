...
//      返回布尔值列表 answer，只有当 xi 可以被 5 整除时，
...
int main()
{
    ...
    int rs = 0;
    bool* ans = PrefixesDivBy5(arr, n, &rs);
    PrintVecElement(arr, n);
    printf("可被 5 整除的二进制前缀 为:\n");
    for(int i = 0;i < rs; i++)
    {
        PrintBool(ans[i]);
    }
...
{
    *returnSize = numsSize;
    bool* ans = (bool*)malloc(sizeof(bool) * (*returnSize));
    memset(ans, 0, sizeof(bool) * (*returnSize));
    int k = 0;
