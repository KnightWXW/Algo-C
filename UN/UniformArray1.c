...
//      链接: https://leetcode.cn/problems/construct-uniform-parity-array-i/

//      给你一个长度为 n 的数组 nums1,其中包含 互不相同 的整数。
//      你需要构造另一个长度为 n 的数组 nums2,
//      使得 nums2 中的元素要么全部为 奇数,要么全部为 偶数。
...
bool UniformArray1(int* nums1, int nums1Size);
...
int main()
{
    ...
    bool ans1A = UniformArray1(nums, n);
    printf("可以构造奇偶一致的数组吗?\n");
    PrintBool(ans1A);
    FreeVec(nums);
}
